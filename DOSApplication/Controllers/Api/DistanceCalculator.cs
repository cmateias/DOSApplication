        ...
        public Dtl.DistanceMatrix Calculate(Models.DistanceRequest request)
        {
            try
            {
                using (var objClient = new WebClient())
                {
                    ...
                    var strUrl = string.Format("https://maps.googleapis.com/maps/api/distancematrix/json?units=imperial&origins={0}&destinations={1}&mode={2}&key={3}", HttpUtility.UrlEncode(request.Origin), HttpUtility.UrlEncode(request.Destination), HttpUtility.UrlEncode(request.Mode.ToLower()), _key);
